...
- `cygma-migration-tools` was updated to version 0.2.6 with an admin-only `sync-design-kit` REST action. The action downloads the exact staging font and hover assets into production uploads and reapplies staging Kit colors, typography, container widths, overflow guard, and button hover CSS to active production Kit 4. Follow-up `0.2.6` CSS hardening forces the staging body/menu typography metrics and the three Roboto Flex home hero words without changing the blue Venture13 hero words.

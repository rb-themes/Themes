...
class Hitboox_Sticky_Module {
    ...
    public function scripts() {
        ...
        wp_register_script('elementor-sticky', get_template_directory_uri() . '/assets/js/vendor/elementor-sticky.js', ['jquery'], HITBOOX_VERSION, true);
        wp_enqueue_script('hitboox-elementor-sticky', get_template_directory_uri() . '/assets/js/vendor/sticky.js', ['jquery', 'elementor-frontend', 'elementor-sticky'], HITBOOX_VERSION, true);
    }
    ...
    public function register_controls(Element_Base $element) {
        $element->add_control(
            'sticky',
            [
                'label'              => esc_html__('Sticky', 'hitboox'),
                'type'               => Controls_Manager::SELECT,
                'options'            => [
                    ''       => esc_html__('None', 'hitboox'),
                    'top'    => esc_html__('Top', 'hitboox'),
                    'bottom' => esc_html__('Bottom', 'hitboox'),
                ],
                'separator'          => 'before',
                'render_type'        => 'none',
        ...
        foreach ($active_devices as $device) {
            $label                          = 'desktop' === $device ? esc_html__('Desktop', 'hitboox') : $active_breakpoint_instances[$device]->get_label();
        ...
        $element->add_control(
            'sticky_on',
            [
                'label'              => esc_html__('Sticky On', 'hitboox'),
        ...
        $element->add_responsive_control(
            'sticky_offset',
            [
                'label'              => esc_html__('Offset', 'hitboox'),
        ...
        $element->add_responsive_control(
            ...
            [
                'label'              => esc_html__('Effects Offset', 'hitboox'),
                'type'               => Controls_Manager::NUMBER,
                'default'            => 0,
                'min'                => 0,
                'max'                => 1000,
        ...
        if ($this->is_instance_of($element, $types)) {
            ...
            $element->add_control(
                'sticky_parent',
                [
                    'label'              => esc_html__('Stay In Column', 'hitboox'),
    ...
    public function register_controls_search_popup(Element_Base $element) {
        ...
        $element->add_control(
            'search_popup_parent',
            [
                'label'        => esc_html__('Parent search popup', 'hitboox'),
                'type'         => Controls_Manager::SWITCHER,
